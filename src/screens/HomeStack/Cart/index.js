...
console.log(OfferData);
  ...
  const fetchCartDetails = async () => {
    try {
      ...
      console.log(data);
      console.log("ksdjsnkdndjsdsjbdsjbdsjhdbshjdsbhjsdbdjshbhjdsb sjhcbjhsdbvhjsdb");
      console.log(mid);
      if (data.success) {
             ...
             <Text style={styles.actualPrice}>{`$${(
              ...
              <Text style={styles.discountedPrice}>{`$${parseFloat(
               ...
               <Text style={styles.discountText}>
                {/* {console.log("dkhsdbhjdsbhdsjvbsjvbshjbvdshjvbshvj",product)} */}
                {product.discount === '' 
    ? null 
    : (product.discount_type === 'amount' 
        ? `S$ ${parseFloat(product?.discount || 0).toFixed(2)} off` 
        : `${parseFloat(product?.discount || 0).toFixed(2)} % off`)}
              </Text>
