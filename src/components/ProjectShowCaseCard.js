...
const ProjectShowCaseCard = ({ data, onPress }) => {
  ...
  useEffect(() => {
    const fetchCategoryData = async () => {
      try {
        // Fetch category name
        const categoryResponse = await fetch(`${BASE_URL}getCategory_sm?cid=${data?.category}`, {
          headers: {
        ...
        if (categoryResult.success) {
          setCategoryName(categoryResult.category_data.ctg_name);
        } else {
          console.error('Failed to fetch category name:', categoryResult.message);
        }
      } catch (error) {
      ...
      //   const allCategoriesResponse = await fetch('https://apis2.devcorps.in/getAllCategories_sm', {
      //     headers: {
      //       'api-key': '90bd6f5b-033f-42e7-8e92-2a443dfa42f8',
      //     },
      //   });
      //   console.log(allCategoriesResponse,"lallsaslsalsallaslaslaslalsalslaslasl");
      //   const allCategoriesResult = await allCategoriesResponse.json();
      //   if (allCategoriesResult.success) {
      //     setCategories(allCategoriesResult.categories || []);
      //   } else {
      //     console.error('Failed to fetch categories:', allCategoriesResult.message);
      //   }
      // } catch (error) {
